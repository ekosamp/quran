...
export default function SurahPage({ surah, previousSurah, nextSurah }) {
  return (
    <>
      <Head>
        <title>
          Surat {surah.name.transliteration.id}
          {" - "}
          {app.name}
        </title>
      </Head>
      ...
      <main className="container content">
        <div className="flex flex-col text-center mt-5 mb-1">
          <span
            className="text-arabic text-2xl text-gray-900 font-semibold"
            dir="rtl"
            lang="ar"
          >
            {surah.name.long}
        ...
        <div className="flex justify-between sticky bottom-2 left-0 right-0 bg-blue-600 text-white z-30 py-3 px-4 shadow-lg rounded-lg max-w-full mx-3 my-4">
          <Link
            href={
              Object.keys(previousSurah).length
                ? `/surah/${previousSurah.number}`
                : "/surah"
            }
          >
            <a className="flex items-center">
              ...
              <span className="text-md hidden sm:inline-block">
                {Object.keys(previousSurah).length
